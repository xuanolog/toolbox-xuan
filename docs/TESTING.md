...
- JVM 测试 63 项全部通过，Lint 0 错误、4 项原有非阻断警告。新增实际 `_1.htm + page.run(1)` 已登录页、`_2.htm + page.run(2)` 注销页、通用成功字符串不能冒充页面状态、跨 IP 拒绝，以及 PHP Cookie 的 HttpOnly 保留、跨主机隔离和操作间隔离测试，并校验注销请求与浏览器实测参数顺序一致。
- 前台 Chrome 只读核对了真正的 `wc/exit/logout_portal` 函数与协议开关，确认浏览器持有 HttpOnly PHPSESSID；未读取输入框或输出 Cookie 值。
- 真机对照：无 Cookie 的相同注销请求返回 `success=false`、“注销失败”；先调用网页会话初始化接口并携带新建 Cookie 的请求返回 `success=true`、“注销成功”。该请求只使用学校公开的占位账号密码，不读取保存的账号配置。
- 随后通过正式 `NetworkPortalSession.readContext()` 访问“返回”入口的真机测试通过（1 项），确认注销后能够取得与当前 Wi-Fi 一致的新登录参数。原有“页面已变化”的解析阻塞已覆盖。
- 用户已确认首次连接显示绿色成功；切换配置仍报“校园注销未成功”，完整换号验收未通过。
- 后续真机复查：正式注销流程失败；相同最小 Cookie 初始化流程也再次返回“注销失败”。Cookie 连续性测试通过，自定义请求头和默认请求头均能保持同一会话；隔离在线查询 Cookie 后仍失败，已撤回该实验改动。因此前次单次成功不足以证明 Cookie 是注销失败的根因，仍需浏览器实际请求对照。
- 测试启动早于前台 Activity 曾导致“Wi-Fi unavailable”；稳定流程为启动测试进程三秒后再打开 Activity。

- 后续通过 Chrome 实际注销请求确认浏览器注销成功；最新源码已将原生请求参数顺序对齐浏览器（callback 在前、v 在末尾）。该调整通过自动测试和构建，但尚未完成真机换号验证，不能认定已修复。

用户要求停止继续修复并打包，故交付当前 `0.1.3` 候选快照。包内 APK 使用原调试签名，可覆盖安装；本次打包未再次更新手机。绿色成功提示已获用户确认，账号切换仍为已知未验收问题，不创建正式验收标签。
